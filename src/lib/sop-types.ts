...
// ── Bells → oz conversion (display/export aid) ─────────────────────
...
// wording. Returns null when the text has no bell measurement, so
// callers can skip annotating cells like "Stir & fill".
const BELL_FRACTIONS: Record<string, number> = { '½': 0.5, '¼': 0.25, '¾': 0.75 };
const BELL_RE = /(\d+(?:\.\d+)?\s*[½¼¾]?|[½¼¾])\s*(small|large|3-oz|3 oz|5-oz|5 oz)\s+bells?/gi;

function parseBellQty(raw: string): number {
  const t = raw.trim();
  const m = t.match(/^(\d+)?\s*([½¼¾])$/);
  if (m) return (m[1] ? parseInt(m[1], 10) : 0) + BELL_FRACTIONS[m[2]];
  return parseFloat(t);
}
